...
namespace Emp37.Tweening
{
      ...
      {
            public static Value<T> Value<T>(Func<T> source, Func<T> destination, float duration, Action<T> update, Value<T>.Interpolator interpolator) where T : struct
            {
                  var tween = Tweening.Value<T>.Fetch(source, destination, duration, update, interpolator);
