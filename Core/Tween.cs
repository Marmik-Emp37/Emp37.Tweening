...
namespace Emp37.Tweening
{
      ...
      public static class Tween
      {
            ...
            private static bool ValidateArguments(UObject link, object initialization, object target, float duration, object update, object evaluator)
            {
                  bool ok = true;
                  if (link == null) { Log.RejectTween($"Missing {nameof(link)}."); ok = false; }
                  if (initialization == null) { Log.RejectTween($"Missing {nameof(initialization)} delegate."); ok = false; }
            ...
            public static Value<T> Value<T>(UObject link, Func<T> initialization, T target, float duration, Action<T> update, Value<T>.Evaluator evaluator) where T : struct => ValidateArguments(link, initialization, target, duration, update, evaluator) ? new Value<T>(link, initialization, target, duration, update, evaluator) : Tweening.Value<T>.Empty;
            public static Value<T> Value<T>(UObject link, Func<T> initialization, Func<T> dynamicTarget, float duration, Action<T> update, Value<T>.Evaluator evaluator) where T : struct => ValidateArguments(link, initialization, dynamicTarget, duration, update, evaluator) ? new Value<T>(link, initialization, dynamicTarget, duration, update, evaluator) : Tweening.Value<T>.Empty;
